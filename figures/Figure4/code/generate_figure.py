...
DIRECTION_LEGEND_Y = 0.990
DIRECTION_LEGEND_FONTSIZE = 13.0
OUTER_TOP = 0.925
OUTER_LEFT = 0.125
ROW_WIDTH_RATIOS = [1.28, 1.42]
ROW_WSPACE = 0.18
ENDPOINT_LABEL_X = -0.32
SHOW_TOP_COEFFICIENTS_SUBTITLE = False
PANEL_E_VERTICAL_SHIFT = -0.018
SHARE_GENE_XAXIS = True
PANEL_LETTER_FONTSIZE = 32
LETTER_X = -0.065
GENE_XLABEL_FONTSIZE = 12.2
GENE_TICK_FONTSIZE = 10.8
GENE_LABEL_FONTSIZE = 11.2
ENDPOINT_FONTSIZE = 18.0
SUBTITLE_FONTSIZE = 14.5
GSEA_XLABEL_FONTSIZE = 10.8
GSEA_YLABEL_FONTSIZE = 15.0
GSEA_TICK_FONTSIZE = 10.6
SIGNIFICANCE_MARKER_FONTSIZE = 19
EXPLORATORY_MARKER_FONTSIZE = 20
HEATMAP_ENDPOINT_FONTSIZE = 14.5
HEATMAP_SELECTED_LABEL_FONTSIZE = 11.0
HEATMAP_LABEL_FONTSIZE = 10.0
HEATMAP_MARKER_FONTSIZE = 19
HEATMAP_TITLE_FONTSIZE = 15.5
COLORBAR_TICK_FONTSIZE = 10.5
COLORBAR_LABEL_FONTSIZE = 12.0
FOOTNOTE_FONTSIZE = 10.5
DISPLAY_GENE_SET_NAMES = {
    **DISPLAY_GENE_SET_NAMES,
    "Pperoxisome": "Peroxisome",
    "Reactive Oxygen Species Pathway": "ROS Pathway",
    "TNF-alpha Signaling via NF-kB": "TNF-alpha Pathway",
}


def _publication_compound_name(value: object) -> str:
    """Restore display capitalization without altering source identifiers."""
    if pd.isna(value):
        return ""
    text = str(value).strip()
    upper = text.upper()
    if upper.startswith("BRD-") or upper in {"CCCP", "MD-920"}:
        return upper
    return text.title()


_original_panel_compound_nes = panel_compound_nes


def _panel_compound_nes_with_display_case(ax, task, gene_set, union_gsea):
    display_frame = union_gsea.copy()
    selected = display_frame["task"].eq(task) & display_frame["gene_set"].eq(gene_set)
    fulvestrant = selected & display_frame["ik"].eq("VWUXBMIQPBEWFH")
    display_frame.loc[fulvestrant, "drug_name"] = "Fulvestrant"
    display_frame.loc[selected, "drug_name"] = display_frame.loc[selected, "drug_name"].map(
        _publication_compound_name
    )
    return _original_panel_compound_nes(ax, task, gene_set, display_frame)


panel_compound_nes = _panel_compound_nes_with_display_case
