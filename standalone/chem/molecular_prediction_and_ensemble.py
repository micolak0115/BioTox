#!/usr/bin/env python3
"""Config-driven standalone molecular modeling and ensemble construction."""
...
def _register_bundled_package(name: str, *locations: Path) -> None:
    ...
    alias_root = ROOT.parent / "_pkg_alias"
...
def _run_cli(script: Path, arguments: list[str], dry_run: bool) -> None:
    ...
    print("[standalone-chem]", " ".join(command))
    if dry_run:
        return
...
def validate(config: dict[str, Any], config_path: Path) -> None:
    ...
    print("[standalone-chem] configuration valid")
...
def _build_cohort(config: dict[str, Any], config_path: Path, dry_run: bool) -> None:
    ...
    print(f"[standalone-chem] build_cohort -> {output}")
    if dry_run:
        return
    module = importlib.import_module("publication.build_matched_cohort")
...
def _fit_candidates(config: dict[str, Any], config_path: Path, dry_run: bool) -> None:
    ...
    print(f"[standalone-chem] fit_candidates -> {output}")
    if dry_run:
        return
    inputs = config["inputs"]
    ...
    os.environ["BIOTOX_CHEMPROP_SCRIPT"] = str(
        CHEM_RUN / "chemprop_stage1_candidate.py"
    )
    module = importlib.import_module("publication.chem_stage1_per_task_offset")
...
def _select_ensemble(config: dict[str, Any], config_path: Path, dry_run: bool) -> None:
    ...
    _run_cli(
        CHEM_RUN / "plot_topk_ensemble_publication.py",
        [
            "--stage1-dir",
...
def _deploy_ensemble(config: dict[str, Any], config_path: Path, dry_run: bool) -> None:
    ...
    _run_cli(
        CHEM_RUN / "deploy_stage1_k4_mean.py",
        [
            "--stage1-dir",
            str(_output_path(config, config_path, "stage1_dir")),
            "--reference",
