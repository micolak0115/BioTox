...
DEFAULT_CONFIG = HERE / "table_paths.json"
...
# Populated by configure_paths() before any build_*()/main() call -- either
# from the CLI entry point below or by a caller (e.g. regenerate_tables.py)
# that imports this module directly.
TOX21_PATH: Path
COHORT_DIR: Path
PAIRED_MANIFEST: Path
PRIMARY_RUN_MANIFEST: Path
ASSAY_METADATA: Path
ENSEMBLE_COMPOSITION: Path
OUTPUT_DIR: Path


def _register_stage1_splitter(splitter_path: Path) -> None:
    """Load splitter.py (currently at publication/pipeline/stage1/splitter.py)
    as `stage1.splitter`. The cohort .pkl files under COHORT_DIR were pickled
    while `DataSplit` lived at that exact dotted path, so unpickling them
    requires `stage1.splitter` to be importable under that name specifically --
    a plain `sys.path` insert of splitter.py's own directory is not enough."""
    if "stage1.splitter" in sys.modules:
        return
    stage1_pkg = types.ModuleType("stage1")
    stage1_pkg.__path__ = []
    spec = importlib.util.spec_from_file_location("stage1.splitter", splitter_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    stage1_pkg.splitter = module
    sys.modules["stage1"] = stage1_pkg
    sys.modules["stage1.splitter"] = module


def configure_paths(config_path: Path = DEFAULT_CONFIG, output_dir: Path | None = None) -> None:
    """Load this script's input paths from `table_paths.json`, register
    `stage1.splitter` (see _register_stage1_splitter), and set OUTPUT_DIR to
    `output_dir` (or a timestamped directory under the config's output_root)."""
    global TOX21_PATH, COHORT_DIR, PAIRED_MANIFEST, PRIMARY_RUN_MANIFEST
    global ASSAY_METADATA, ENSEMBLE_COMPOSITION, OUTPUT_DIR
    raw = json.loads(config_path.read_text(encoding="utf-8"))
    config = raw["generate_supplementary_tables"]
    TOX21_PATH = Path(config["tox21_smiles"])
    COHORT_DIR = Path(config["cohort_dir"])
    PAIRED_MANIFEST = Path(config["paired_manifest"])
    PRIMARY_RUN_MANIFEST = Path(config["primary_run_manifest"])
    ASSAY_METADATA = Path(config["assay_metadata"])
    ENSEMBLE_COMPOSITION = Path(config["ensemble_composition"])
    _register_stage1_splitter(Path(config["stage1_splitter_dir"]) / "splitter.py")
    if output_dir is not None:
        OUTPUT_DIR = output_dir
    else:
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        OUTPUT_DIR = Path(raw["output_root"]) / f"generate_supplementary_tables_{stamp}"
...
def build_dataset_flow() -> pd.DataFrame:
    from stage1.splitter import generate_scaffold
...
def build_stage1_hyperparameters() -> pd.DataFrame:
    source_classical = "chem/classical_baseline.py"
    source_stage1 = "publication/chem_stage1_per_task_offset.py"
    source_chemprop = "chem/chemprop_stage1_candidate.py"
    rows = [
        {
            "candidate": "MACCS_LR",
...
```bash
/data/kyungan/miniconda3/envs/BioTox/bin/python \\
  build_publication_methods_tables_v1.py \\
