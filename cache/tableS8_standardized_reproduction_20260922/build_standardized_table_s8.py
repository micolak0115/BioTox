...
sys.path.insert(0, "/home/kyungan/scripts/BioTox/publication/pipeline/stage2")
...
RUN = Path("/data/kyungan/scripts/BioTox/publication/pipeline/stage2/methodology_audit/results/primary_6h_20x5")
OUT = Path("/data/kyungan/scripts/BioTox/publication/analyses/tableS8_standardized_reproduction_20260922")
OLD = pd.read_csv("/home/kyungan/scripts/BioTox/publication/tables/TableS8/source_data/TableS8_source_data.csv")
...
for cell in CELLS:
    mask = stage1.cell.eq(cell)
    stage1.loc[mask, "panel_a_bh_q_value"] = bh(stage1.loc[mask, "panel_a_nb_p_value"])
    stage1.loc[mask, "panel_a_auprc_bh_q_value"] = bh(stage1.loc[mask, "panel_a_auprc_nb_p_value"])
...
new = stage1.merge(panel_b, on=["task", "cell"], validate="one_to_one").sort_values(["task", "cell"])
