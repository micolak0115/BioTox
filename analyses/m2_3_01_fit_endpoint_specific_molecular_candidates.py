...
Design:
  ...
  2. 12 candidates per task: MACCS/Morgan/RDKit x LogReg/XGBoost (6) +
     ...
     Chemprop/D-MPNN (1, subprocess into the dedicated crisp_chemprop_env
     -- chemprop==2.2.2 pins torch/lightning versions incompatible with
     this repo's main crisp_env; see chem/chemprop_stage1_candidate.py).
...
the multi-task (12-head) architecture chem/build_chem_offset_ensemble.py
...
sys.path.insert(0, "/data/kyungan/scripts/BioTox")

from chem.benchmark import _resolve_pretrained_gnn_checkpoint, _load_pretrained_gnn_backbone, _run_seed_jobs  # noqa: E402
from chem.classical_baseline import CLASSICAL_VARIANTS, _fit_predict_one_task, featurize  # noqa: E402
from chem.gnn_matched_benchmark import _build_loader  # noqa: E402
from chem.model import ChemBerta, GAT, GCN, GIN, GraphSAGE  # noqa: E402
from chem.splitter import DataSplit  # noqa: E402
from chem.trainer import compute_pos_weight  # noqa: E402
from chem.utils import seed_all  # noqa: E402
from publication.utils import CELL_IDS, TOX21_TASKS  # noqa: E402
...
# concurrently via joblib (chem.benchmark._run_seed_jobs, loky backend --
...
def fit_predict_classical(fp_kind: str, clf_kind: str, fit_df, predict_df, task: str, seed: int,
    ...
    cross-candidate comparison (see chem/classical_baseline.py::_fit_predict_one_task)."""
...
def fit_predict_gnn_pretrained(arch: str, fit_df, es_df, predict_df, task: str, seed: int, gpu_id: int = -1) -> np.ndarray:
    ...
    seed_all(seed)
    fit_loader = _build_loader(fit_df, [task], "smiles_canon", 32, shuffle=True)
    es_loader = _build_loader(es_df, [task], "smiles_canon", 32, shuffle=False)
    pred_loader = _build_loader(predict_df, [task], "smiles_canon", 32, shuffle=False)
    ...
    model = GNN_CLASSES[arch](
        input_dim=ATOM_FEATURE_DIM, hidden_dim=300, output_dim=1, n_layers=5,
        dropout=0.5, graph_pooling="mean", JK="last", task_cols=[task],
        task_family_heads=False, task_mode="binary_classification",
        lr=1e-4, epochs=100, seed=seed, save_path=None,
        patience=20, min_delta=0.0, device=device,
    )
    checkpoint_path = _resolve_pretrained_gnn_checkpoint({}, arch)
...
def fit_predict_chemberta_pretrained(fit_df, es_df, predict_df, task: str, seed: int, gpu_id: int = -1) -> np.ndarray:
    from chem.finetune import predict_smiles
    ...
    pos_weight = compute_pos_weight(fit_df[task].to_numpy(dtype=np.float64), "binary_classification")
    model = ChemBerta(
        model_name=ChemBerta.DEFAULT_MODEL, task_mode="binary_classification", num_labels=1,
        max_length=128, frozen=False, smiles_col="smiles_canon", task_cols=[task],
        task_family_heads=False, seed=seed, lr=1e-4, epochs=20, patience=5,
        batch_size=32, save_path=None, trust_remote_code=True, device=device,
    )
    fit_split = DataSplit(train=fit_df, valid=es_df, test=es_df)
    model = model.fit(fit_split, pos_weight=pos_weight)
    result = predict_smiles(model, predict_df["smiles_canon"].tolist(), batch_size=32, max_length=128)
    return np.asarray(result["probs"]).reshape(-1)


CHEMPROP_ENV_PYTHON = "/data/kyungan/miniconda3/envs/crisp_chemprop_env/bin/python"
CHEMPROP_SCRIPT = "/data/kyungan/scripts/BioTox/chem/chemprop_stage1_candidate.py"


def fit_predict_chemprop(fit_df, es_df, predict_df, task: str, seed: int, gpu_id: int = -1) -> np.ndarray:
    """Chemprop (D-MPNN) runs in a SEPARATE conda env (`crisp_chemprop_env`
    -- chemprop==2.2.2 pins torch==2.6.0/lightning==2.6.0, incompatible
    with this repo's main `crisp_env` used for every other candidate here),
    invoked as a subprocess. Serializes fit/es/predict to temp CSVs
    (chem/chemprop_stage1_candidate.py's interface), reads predictions
