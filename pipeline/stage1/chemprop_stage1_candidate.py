# stage1/chemprop_stage1_candidate.py
"""
Chemprop (D-MPNN) single-task candidate for publication/chem_stage1_per_task_offset.py.

Runs in the DEDICATED `BioTox_chemprop` conda env (chemprop==2.2.2 pins
torch==2.6.0/lightning==2.6.0, incompatible with this repo's main
`BioTox`, pinned to torch==2.2.2 for the rest of the GNN/ChemBERTa
pipeline) -- invoked as a subprocess by the caller running in `BioTox`,
never imported directly from there.

Deliberately NOT a reuse of stage1/chemprop_baseline.py's `_run_chemprop_seed`:
...
Usage (invoked by fit_predict_chemprop() in chem_stage1_per_task_offset.py):
  conda run -n BioTox_chemprop python stage1/chemprop_stage1_candidate.py \
