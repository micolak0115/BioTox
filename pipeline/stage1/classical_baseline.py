...
Unlike stage1/model.py's `Fingerprint` (a trainable nn.Module + neural
...
follows the same cached/remaining-seed convention as stage1/benchmark.py's
...
stage1/trainer.py::compute_metrics produces, so results_to_df/summarize_results
...
413/137/137 "label_balanced_scaffold" split stage1/eval_matched_137.py and
stage1/main.py's configs use (a different partition, zero valid/test ik
...
from stage1.benchmark import (  # noqa: E402
...
from stage1.trainer import compute_metrics  # noqa: E402
from stage2.utils import load_matched_split  # noqa: E402  (linear/ no longer exists; see publication/METHODS.md)
...
def featurize(smiles_list: list, kind: str) -> np.ndarray:
    ...
    matching Fingerprint.encode's fallback in stage1/model.py."""
    ...
    for split_name, n in expected.items():
        ...
        if actual != n:
            raise ValueError(
                ...
                "413/137/137 label_balanced_scaffold split used elsewhere in stage1/."
            )
...
def classical_benchmark(config: dict, matched_split_path: str, seeds: list, models: Optional[list] = None) -> dict:
    ...
    following the same cached/remaining-seed loop as stage1/benchmark.py's
