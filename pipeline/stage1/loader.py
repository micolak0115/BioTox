...
from stage1.utils import load_pickle, save_pickle
from stage1.splitter import (
    DataSplit, 
    generate_scaffold,
...
def expand_biochem_split_to_full_chem(
    ...
    # 1. Build scaffold → fold from stage2+chem split.
    ...
    print(
        ...
        f"  scaffold_to_fold: {len(scaffold_to_fold)} scaffolds from stage2+chem split\n"
        f"  train={len(train_df)} | val={len(valid_df)} | test={len(test_df)}"
    )
