...
from stage2 import nested_cv_offset_logistic_calibrated_repeated as repeated
from stage2 import nested_cv_offset_logistic_calibrated as calibrated
from stage2 import run_repeated_calibrated_timepoint as timepoint
...
def write_inner_seed_registry(output_root: Path) -> None:
    ...
    n_outer_records = registry[identity].drop_duplicates().shape[0]
    n_variants = registry["variant"].nunique()
    expected_variant_rows = n_outer_records * n_variants
    if len(registry) != expected_variant_rows:
        raise RuntimeError(
            f"Expected {expected_variant_rows} variant-specific inner-seed "
            f"rows for {n_outer_records} outer records and {n_variants} "
            f"variant(s), found {len(registry)}"
        )
    ...
    expected_outer_folds = n_outer_records
    ...
    policy = {
        ...
        "outer_partition_policy": (
            "unchanged from repeated calibrated v3"
        ),
