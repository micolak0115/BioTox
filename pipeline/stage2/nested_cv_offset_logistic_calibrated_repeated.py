...
This module reuses the fold-local fixed-calibration outer-fold fitter:

    logit(P(Y=1)) = eta_chem + intercept + X_bio @ beta

The Stage-1 chemical logit is a globally frozen offset with coefficient one,
the intercept is fitted first and held fixed during beta-only ridge
...
from stage2 import nested_cv_offset_logistic_calibrated as calibrated
from stage2.repeated_cv_inference import (
    repeated_cv_nadeau_bengio_test,
)
from stage2.utils import stratified_scaffold_group_kfold

# Public configuration aliases for pipeline entry points. The repeated engine
# owns the repeated-run contract while the single-fold module remains its
# implementation dependency; callers do not need to import both modules.
CELL_IDS = calibrated.CELL_IDS
TOX21_TASKS = calibrated.TOX21_TASKS
CHEM_OFFSET_PATH = calibrated.CHEM_OFFSET_PATH
COHORT_DIR = calibrated.COHORT_DIR
GENE_FEATURES_DIR = calibrated.GENE_FEATURES_DIR
...
PIPELINE_NAME = "calibrated_stage2_repeated_nested_cv_fixed_calibration_v3"
...
def run_repeated_analysis(
    ...
    *,
    ...
    if use_sample_weight:
        raise ValueError("The Methods specification requires unweighted BCE")
    ...
    manifest: dict[str, Any] = {
        ...
        "alpha_selection_rule": "largest_grid_value_within_one_se_of_global_minimum",
        "validation_loss": "unpenalized_mean_binary_cross_entropy_from_logits",
        "stage2_likelihood": (
    ...
    print(
        f"[repeated-calibrated-v3] {len(repeated_jobs)}/{expected_jobs} "
        f"outer-fold jobs prepared; repeats={len(outer_seeds)}; "
        f"workers={resolved_jobs}; solver={solver}"
    )
    if resolved_jobs == 1:
        outcomes = []
        progress_started = datetime.now(timezone.utc)
        for job_index, repeated_job in enumerate(repeated_jobs, start=1):
            outcomes.append(calibrated.run_outer_fold(repeated_job.job))
            if job_index == 1 or job_index % 10 == 0 or job_index == len(repeated_jobs):
                elapsed = (datetime.now(timezone.utc) - progress_started).total_seconds()
                rate = job_index / elapsed if elapsed > 0 else 0.0
                remaining = len(repeated_jobs) - job_index
                eta_seconds = remaining / rate if rate > 0 else None
                eta_text = f"{eta_seconds:.1f}" if eta_seconds is not None else "unknown"
                print(
                    "[repeated-calibrated-v3] serial progress: "
                    f"{job_index}/{len(repeated_jobs)} outer-fold jobs completed; "
                    f"elapsed_seconds={elapsed:.1f}; eta_seconds={eta_text}",
                    flush=True,
                )
    else:
