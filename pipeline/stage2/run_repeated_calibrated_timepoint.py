...
def _register_bundled_package(name: str, *locations: Path) -> None:
    ...
    original package name -- both in this process and in any subprocess or
    multiprocessing worker it spawns. Same bootstrap as
    run_repeated_calibrated_pipeline.py, duplicated here to match this
    codebase's existing per-script convention (this script previously did a
    plain ``sys.path.insert(0, HERE.parent)``, which only works if some other
    script has already built the alias in-process first)."""
...
from stage2.nested_cv_offset_logistic_calibrated_repeated import (  # noqa: E402
    DEFAULT_OUTER_SEEDS,
...
from stage2.summarize_repeated_cv_publication import summarize  # noqa: E402
from stage2.utils import load_matched_split  # noqa: E402


DEFAULT_INPUT_ROOT = (
    _GENERATED_ROOT / "transcriptomic_residual_learning" / "paired_inputs_6h24h"
)
CHEM_OFFSET_PATH = (
    _GENERATED_ROOT / "molecular_prediction_and_ensemble" / "deployed_k4" / "chem_offset_final.csv"
)
...
def validate_inputs(
    *,
    ...
    # Feature manifests produced before the standalone bundle was assembled
    # may retain an absolute publication/_run_output path.  When that path is
    # stale, the cohort and feature file checksums below are the authoritative
    # identity check; only reject a live path that points at another cohort.
    recorded_cohort_dir = Path(feature_manifest.get("cohort_dir", ""))
    if recorded_cohort_dir.exists() and recorded_cohort_dir.resolve() != (
        cohort_dir.resolve()
    ):
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print("[paired-time-v3] dry run complete; no output created.")
        return
    ...
    print(f"[paired-time-v3] complete: {output_root}")
    if not args.skip_summary:
        print(f"[paired-time-v3] summary: {summary_root}")
