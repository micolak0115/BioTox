...
from stage2 import nested_cv_offset_logistic_calibrated as calibrated  # noqa: E402
from stage2.offset_ridge_fixed import (  # noqa: E402
    FixedOffsetRidgeFit,
    fit_fixed_offset_ridge_lbfgs,
    fit_fixed_offset_ridge_path,
    predict_fixed_offset_ridge,
)
from stage2.offset_ridge_lbfgs import fit_offset_intercept_lbfgs  # noqa: E402
from stage2.repeated_cv_inference import (  # noqa: E402
    repeated_cv_nadeau_bengio_test,
)
from stage2.utils import (  # noqa: E402
...
SOURCE_RUN_DIR = (
    ...
    / "data" / "generated" / "transcriptomic_residual_learning"
    / "primary_6h_20x5"
)
DEFAULT_OUT_DIR = (
    _STANDALONE_ROOT.parent
    / "data" / "generated" / "transcriptomic_residual_learning"
    / "calibration_sensitivity_20x5"
)
SOURCE_GENE_FEATURES_DIR = (
    _STANDALONE_ROOT.parent / "data" / "generated" / "transcriptomic_residual_learning" / "gene_features_6h"
)
MODEL_SPEC = "fixed_chemical_offset_fixed_calibration_intercept_ridge_biology_v3"
CONTRASTS = (
    ("c1_minus_c0", "c1", "c0"),
    ("d1_minus_d0", "d1", "d0"),
    ("d1_minus_c1", "d1", "c1"),
...
def _run_fold(job: FoldJob) -> FoldOutcome:
    ...
    try:
        with threadpool_limits(limits=1):
            ...
            elif scale_ref_train is not None:
                # Fair before/after comparison: standardize the
                # viability-adjusted (residualized) features using the
                # SAME train-fold mean/std as the unadjusted counterpart,
                # rather than leaving them unstandardized.
                mu = scale_ref_train.mean(axis=0)
                sd = scale_ref_train.std(axis=0)
                sd = np.where(sd < 1e-8, 1.0, sd)
                X_train, X_test = (X_train_raw - mu) / sd, (X_test_raw - mu) / sd
            else:
            ...
            # Search across deterministic seeds (matching the primary
            # residual-CV pipeline's _find_binary_evaluable_inner_folds)
            # rather than trying a single fixed seed and failing outright.
            # Endpoint-cell combinations with few positives (e.g. HEPG2
            # for endpoints with under ~40 positives cohort-wide) can have
            # an outer-training partition whose single-seed 3-fold split
            # puts all positives (or all negatives) of one scaffold group
            # into a single inner fold; retrying with nearby seeds finds a
            # split where every inner fold remains binary-evaluable.
            inner_folds, inner_seed_used = calibrated._find_binary_evaluable_inner_folds(
                smiles=smiles_train,
                y=y_train,
                k=calibrated.INNER_CV_K,
                requested_seed=calibrated.INNER_CV_SEED,
            )
            ...
            for inner_fold, (inner_train_idx, inner_valid_idx) in enumerate(
                ...
                elif scale_ref_inner_train is not None:
                    mu = scale_ref_inner_train.mean(axis=0)
                    sd = scale_ref_inner_train.std(axis=0)
                    sd = np.where(sd < 1e-8, 1.0, sd)
                    X_inner_train, X_inner_valid = (
                        (X_inner_train_raw - mu) / sd,
                        (X_inner_valid_raw - mu) / sd,
                    )
                else:
                    X_inner_train, X_inner_valid = (
                        X_inner_train_raw,
            ...
            fold_row: dict[str, Any] = {
                ...
                "inner_cv_seed_requested": int(calibrated.INNER_CV_SEED),
...
def run(args: argparse.Namespace) -> None:
    ...
    manifest = {
        ...
        "pipeline": "stage2_calibration_ridge_ablation_v3",
