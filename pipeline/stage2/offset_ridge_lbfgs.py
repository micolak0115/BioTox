"""Offset-only calibration and the historical joint intercept/ridge solver.

The Methods-compliant Stage-2 pipeline uses ``fit_offset_intercept_lbfgs``
from this module, then freezes that intercept and fits beta with
``offset_ridge_fixed.py``. The joint solver below remains available for
historical comparisons; it is not the current primary residual model.
