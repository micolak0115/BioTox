...
def _register_chem_splitter(splitter_path: Path) -> None:
    """Load splitter.py (currently at publication/standalone/chem/run/splitter.py)
    as `chem.splitter`. The cohort .pkl files under COHORT_DIR were pickled
    while `DataSplit` lived at that exact dotted path, so unpickling them
    requires `chem.splitter` to be importable under that name specifically --
    a plain `sys.path` insert of splitter.py's own directory is not enough."""
    if "chem.splitter" in sys.modules:
        return
    chem_pkg = types.ModuleType("chem")
    chem_pkg.__path__ = []
    spec = importlib.util.spec_from_file_location("chem.splitter", splitter_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    chem_pkg.splitter = module
    sys.modules["chem"] = chem_pkg
    sys.modules["chem.splitter"] = module
...
def configure_paths(config_path: Path = DEFAULT_CONFIG, output_dir: Path | None = None) -> None:
    ...
    `chem.splitter` (see _register_chem_splitter), and set OUTPUT_DIR to
    ...
    _register_chem_splitter(Path(config["chem_splitter_dir"]) / "splitter.py")
...
def build_dataset_flow() -> pd.DataFrame:
    from chem.splitter import generate_scaffold
...
```bash
/data/kyungan/miniconda3/envs/crisp_env/bin/python \\
